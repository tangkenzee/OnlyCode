...
app.get(
    '/api/matchmaking/skills', (req, res) = > {
      ...
      const userMatches = availableUsers.map(user = > {
        ...
        return {
          user : {
            ...
            tagSkillLevels : user.tagSkillLevels || {},
          },
...
app.post(
    '/api/help-requests/:requestId/accept', (req, res) = > {
...
app.get(
    '/api/leaderboard', (req, res) = > {
      const limit = parseInt(req.query.limit) || 50;
      res.json(leaderboard.slice(0, limit));
    });

app.get(
    '/api/stats/global', (req, res) = > {
      res.json({
...
app.post(
    '/api/execute-code', async(req, res) = > {
      ...
      try {
        ...
        const submitResponse =
            await axios.post(`${JUDGE0_BASE_URL} / submissions`, {
              source_code : code,
        ...
        while (attempts < maxAttempts) {
          ...
          const statusResponse =
              await axios.get(`${JUDGE0_BASE_URL} / submissions /
                                  $ { submissionToken }`,
                              {
                                headers : {
                                  'X-RapidAPI-Key' : JUDGE0_API_KEY,
          ...
          if (result.status.id >
              2) {  // Status > 2 means processing is complete
            break;
          }
                  ...
                  >
                  {
                    ...
                    if (index == = 0 && result.status.id == = 3) {
                      ...
                      try {
                        ...
                        passed = JSON.stringify(actualJson) ==
                            = JSON.stringify(expectedJson);
                      } catch {
                        // Compare as arrays of CSV values
                        const extractCSV = (str) =
                            > str.replace(/\[|\] |\s |\n / g, '').trim();
                        const actualCSV = extractCSV(actual).split(',');
                        const expectedCSV = extractCSV(expected).split(',');
                        console.log('DEBUG:',
                                    {actual, expected, actualCSV, expectedCSV});
                        if (actual&& expected&& actualCSV.length ==
                            = expectedCSV.length &&
                              actualCSV.every((v, i) =
                                                  > v == = expectedCSV[i])) {
                          passed = true;
                        } else {
                          // Fallback to whitespace-insensitive string
                          // comparison
                          passed = actual ?.replace(/\s + / g, '') == = expected
                                           ?.replace(/\s + / g, '');
                        }
                      }
                    }
